...
namespace jedjoud.VoxelTerrain.Generation {
    ...
    public class TextureSampler<T> {
        ...
        public Variable<T> scale;
        public Variable<T> offset;
        public delegate Variable<float4> CustomSampling(SampleableTexture<T> texture, Variable<T> coords);
        public CustomSampling function;
        ...
        public Variable<float4> Sample(Variable<T> input) {
            ...
            if (texture.dimension == UnityEngine.Rendering.TextureDimension.Tex2D && VariableType.Dimensionality<T>() != 2) {
                throw new Exception("TextureSampler input coordinate dimension is 2D, but texture dimension isn't 2D");
            }

            if (texture.dimension == UnityEngine.Rendering.TextureDimension.Tex3D && VariableType.Dimensionality<T>() != 3) {
